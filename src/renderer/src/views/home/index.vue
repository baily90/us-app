<template>
  <el-container class="w-100vw h-100vh bg-black">
    <el-header class="header">
      ...
      <div class="body">{{ bodyPartMap[info?.body_part] }}{{ bodySectionMap[info?.current_scan_section] ?
        `--${bodySectionMap[info?.current_scan_section]}` : '' }}</div>
      <div class="header-right">
        <el-button link @click="settingDialog.open('ultrasound')">
          <Icon icon="lets-icons:setting-alt-line" class="text-24px" />
        </el-button>
      </div>
    </el-header>
    <el-container class="w-full">
      ...
      <el-main class="w-[760px] !px-0 !pr-40px">
        ...
        <div class="container-example">
          <!-- 教学示意图 -->
          <!-- :src="getAssetsFile(`./../../assets/images/img${info?.current_scan_section}.jpeg`)" -->
          <img class="example-gif" :src="getAssetsFile(`./../../assets/images/img_test.jpeg`)" alt="">
          <!-- 影像示意图 -->
          <!-- :src="getAssetsFile(`./../../assets/images/gif${info?.current_scan_section}.gif`)" -->
          <img class="example-image" :src="getAssetsFile(`./../../assets/images/gif_test.gif`)" alt="">
          <!-- 指引箭头 -->
...
const tips = computed(() => {
  if (!info.value?.current_scan_section) {
    // 未检测到切面 则提示失败信息
    return { type: 'error', msg: 'No section detected; please confirm the probe is placed as shown' }
...
const scan_progress = computed(() => {
  const key = info.value?.body_part === 1 ? 'thyroid_scan_status' : info.value?.body_part === 2 ? 'carotid_artery_scan_status' : ''
