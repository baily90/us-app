...
export default class ClickMarkObject extends MarkObject {
    ...
    if (this.mouseDown && this.status == "edit") {
      ...
      if (this.acctivePointIndex == -1) {
        ...
        this.pointList = this.pointList.map((point) => {
          // code here...
          // const newPoint = this.forbidOut({x: point.x + offset.x, y: point.y + offset.y})
          return {
            ...point,
            x: point.x + offset.x,
            y: point.y + offset.y,
        ...
        // 更新点位
        const newPoint = this.forbidOut({x: this.lastMousePoint!.x, y: this.lastMousePoint!.y})
        this.pointList[this.acctivePointIndex] = {x: newPoint.x, y: newPoint.y}
      }
  ...
  getTruePointArr() {
    const arr = this.pointList.filter((item: any) => {
      return !item.fictitious;
    });
    return arr.length === 0 ? this.pointList:arr;
  }

  removeFictitiousPoints() {
    this.pointList = this.getTruePointArr();
  }
  ...
  render() {
    ...
    if (this.status === "draw") {
      ...
      const drawPath = new Path2D(path);
    ...
    if (this.status === "edit") {
      this.box.clearCanvas(ctx);
      const drawPath = new Path2D(this.pathData);
      this.group.push(drawPath);
      ctx.stroke(drawPath);
      ctx.fillStyle =
      ...
      this.pointList.map((item, index) => {
        let circle = null;
        const radius = this.acctivePointIndex === index ? 8 : 4
        if (item.fictitious) {
          ctx.beginPath()
          ctx.arc(item.x, item.y, radius / zoom, 0, 2 * Math.PI)
          ctx.fillStyle = 'white' // 使用背景颜色
          ctx.fill()
        }
  ...
  isPointInside(point: IPointData): boolean {
    ...
    const offset = isPointInPolygon(point, this.pointList);
