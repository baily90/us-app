...
export default class CanvasMarkBoard implements ICanvasMarkBoard {
  ...
  init(config: IMarkBoardConfig) {
    ...
    this.regionCtx = this.regionCanvas.getContext('2d') as CanvasRenderingContext2D
  ...
  setLayout({ width, height }: { width: number; height: number }) {
    const zoomx = this.viewDomInfo.width / width
    const zoomy = this.viewDomInfo.height / height
    const zoom = Math.min(zoomx, zoomy)
    const offsetx = (this.viewDomInfo.width - width * zoom) / 2
    const offsety = (this.viewDomInfo.height - height * zoom) / 2
    this.t = {
      a: zoom,
      b: 0,
      c: 0,
  ...
  getDistance(p1, p2) {
    return Math.sqrt(
      Math.pow((p1.x - p2.x) * this.config.mm_per_pixel, 2) +
        Math.pow((p1.y - p2.y) * this.config.mm_per_pixel, 2),
    )
  }
  /** TODO:优化全部渲染 */
  render() {
    for (let i = 0; i < this.renderGroup.length; i++) {
    ...
    this.markObjectList.map((item) => {
      if (item.status !== 'draw' && this.config.showIndex) {
        ...
        if (item.type === 'ruler') {
          const distance = this.getDistance(item.pointList[0], item.pointList[1])
          this.ctx.fillText(
          ...
          this.ctx.fillText(item.label, item.indexPoint.x, item.indexPoint.y - 3 / this.t.a)
        }
      }
  ...
  /** 画布不能编辑事件 */
  setMarkDisable(isDisabled = false) {
  ...
  appMoving(point: IPointData) {
    ...
    const moveX = (point.x - this.lastMovePoint.x) * this.t.a
    const moveY = (point.y - this.lastMovePoint.y) * this.t.a
    this.t.e += moveX
    this.t.f += moveY
    this.transfrom()
  ...
  appMousemove(e: MouseEvent) {
    if (this.config.disable) return
    const point = this.pointMapping(e)
    ...
    const drawStatus = !!this.markObjectList.find((item) => item.status === 'draw')
    // 根据状态设置光标
    if (drawStatus) {
    ...
    e.buttons !== undefined && (this.lastMovePoint = { x: point.x, y: point.y }),
  ...
  /**绘制的和选中的才触发事件 */
  getDrawMark(method: 'boxMousedown' | 'boxMousemove' | 'boxMouseup', point: IPointData): void {
    if (this.config.disable) return
  ...
  appMouseup(e: MouseEvent) {
    ...
    if (this.selectObject && this.drag) {
      if (!this.selectObject.isPointInside(point) && this.selectObject.acctivePointIndex === -1) {
    ...
    const lastMark = this.markObjectList[this.markObjectList.length - 1]
    if (
      ...
      const index = this.getSelectedIndex(point)
  ...
  getSelectedIndex(point: IPointData) {
    if (this.config.disable) return
    const indexs: number[] = this.markObjectList.map((_, i) => i).reverse()
    const indexList = indexs.filter((i) => {
  ...
  get objects(): IMarkObjectInfo[] {
    const data = this.markObjectList
      .filter((item) => item.status !== 'draw')
      .map((obj) => {
        let pointXList: any[] = []
        const arr = obj.resultPoints || obj.pointList
        pointXList = arr.filter((item: any) => !item.fictitious)
        return {
          id: obj.id,
          label: obj.label,
          type: obj.type,
  ...
  public addObjectData() {
    ...
    if (this.currentDrawingType) {
      try {
        ...
        throw new Error(`${this.currentDrawingType} mark type is not supported`)
      }
    }
  ...
  on_(_type: string, _listener: IEventListener, _bind?: IObject): IEventListenerId {
